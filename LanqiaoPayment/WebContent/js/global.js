...
! function(G, $) {
	...
	var topEle = {
		...
		// 吊顶未登录显示节点
		unlogin: [
			'<li class="self">欢迎使用蓝桥银行金融服务平台</li>',
			'<li>',
			'    <a href="' + Global.Domain.login + '" target="_blank">登录</a>',
			'    /',
			'    <a href="' + Global.Domain.reg + '" target="_blank">注册</a>',
			'</li>'
		].join("")
	...
	G.Header.init = function() {
		 ...
		 *
				// 加载当前登录用户的服务器地址
				var loadUrl = Global.Domain.passport + '/member/load?' + G.Config.callback;

				// 取得当前用户是否登录
				// 已登录用返回用户名称
				$.getJSON(loadUrl, function(username) {
					if (username == '') {
